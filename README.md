...
This project is an Uber dashboard created using Power BI to analyze ride data. It shows important details like total rides, revenue, trip distance, and customer trends in a clear and visual way. This dashboard helps to understand business performance and make better decisions easily.
...
Table of contents
Project overview
Files & images included
Data model (star schema)
Data preparation & transformations (step‑by‑step)
DAX measures (full code & explanation)
Dashboard pages & visualisations (detailed point‑by‑point)
Navigation, bookmarks & buttons (how to add PNG as page navigator)
Performance recommendations
How to reproduce / deliverables
...
Import the Trip Details, Location Table and Calendar into Power BI Desktop (Get Data → Excel).
...
Calendar table: Use your 30‑row calendar or create a full date table if you need longer date ranges. Include DayName and DayNum columns.

Load cleaned tables to the model.

Set relationships in Model view (see section 3). Mark Calendar as date table if relevant (Mark as Date Table).

Create indexes (if necessary) or reduce cardinality: convert high-cardinality text to keys where possible.
...
Import your PNG (e.g. small home icon) into Power BI (Insert → Image).
With the image selected, create a bookmark that captures the current page or a filtered state.
Go to View → Bookmarks Pane. Create a bookmark (e.g. Overview_State).
While the image is selected, go to Action in the Visualizations pane, turn Action on, set Type to Bookmark and pick the bookmark you created.
Repeat for other page images (Time, Details). Place all page images in a left rail for consistent navigation. You can also set Back behavior on a separate back-arrow PNG.
...
Files to deliver: Power BI .pbix, exported README.md (this file), relationship diagram PNG, images used for page navigation (PNG icons), and an export CSV of sample rows.
...
Publish to Power BI Service if you want web sharing, and configure dataset refresh if source is live.
